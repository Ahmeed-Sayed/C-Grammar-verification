...
public class replace extends gramBaseVisitor{
 ...
 /* @Override public Integer visitStructOrUnionSpecifier(gramParser.StructOrUnionSpecifierContext ctx) {


        try {
            Object obj = parser.parse(new FileReader("myJSON.json"));
            JSONObject jsonObject = (JSONObject) obj;
            regex = (String) jsonObject.get("IDStruct");
        ...
        if (!matchFound) {
            System.out.println("Struct Syntax not Correct at line: " + line);
    ...
    @Override public Integer visitEnumSpecifier(gramParser.EnumSpecifierContext ctx) {   // checks enum syntax , doesn't work with global you have to disable it .
        ...
        boolean matchFound = matcher.find();
        if (!matchFound) {
            System.out.println("enum Syntax not Correct at line: " + line);
    ...
    /*@Override public Integer visitFunctiondeclaration(gramParser.FunctiondeclarationContext ctx) {
...
*/
  /* @Override public Integer visitGlobaldeclaration(gramParser.GlobaldeclarationContext ctx) {
        ...
        String regex = null;
            ...
            boolean matchFound = matcher.find();
            if (!matchFound) {
                System.out.println(type2 + " global syntax not Correct at line: " + line);
            }

/*else {
            String type  = ctx.declaration().declarationSpecifiers().getChild(0).getText();
            int i =0;
            int idEnd=5;
            String ID="";
            String type2="";
            ...
            if (type.substring(0,5)=="struct"){
                 ID= type.substring(6,idEnd);
                 type2="struct";

            }else if (type.substring(0,4)=="union"){
                ID= type.substring(5,idEnd);
                type2="union";
            }else if (type.substring(0,3)=="enum"){
                 ID= type.substring(4,idEnd);
                type2="enum";
            }
            try {
                Object obj = parser.parse(new FileReader("myJSON.json"));
            ...
            boolean matchFound = matcher.find();
            if (!matchFound) {
                System.out.println(type2+" Syntax not Correct at line: " + line);
...
return 0;
   ...
   @Override public Integer visitLocaldeclaration(gramParser.LocaldeclarationContext ctx) {            // checks Local enum,union,struct, type variables syntax
      ...
      JSONParser parser = new JSONParser();
       ...
       boolean matchFound = matcher.find();
       if (!matchFound) {
           System.out.println(type2 + " local syntax not Correct at line: " + line);
       }
...
return 0;}
    ...
    @Override public Integer visitStructOrUniontype (gramParser.StructOrUniontypeContext ctx) {    // checks global struct or union declarator syntax
        ...
        boolean matchFound = matcher.find();
        if (!matchFound) {
            System.out.println(type +" Syntax not Correct at line: " + line);
   ...
   /* @Override public Integer visitFunctionDefinition(gramParser.FunctionDefinitionContext ctx) {
        ...
        boolean matchFound = matcher.find();
        if (!matchFound) {
            System.out.println("Function  Syntax not Correct at line: " + line);
